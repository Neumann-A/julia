...
function create_match_context()
    ...
    jit_stack = ccall((:pcre2_jit_stack_create_8, libpcre2), Ptr{Cvoid},
                      (Csize_t, Csize_t, Ptr{Cvoid}),
                      JIT_STACK_START_SIZE, JIT_STACK_MAX_SIZE, C_NULL)
    ctx = ccall((:pcre2_match_context_create_8, libpcre2),
                Ptr{Cvoid}, (Ptr{Cvoid},), C_NULL)
    ccall((:pcre2_jit_stack_assign_8, libpcre2), Cvoid,
...
function info(regex::Ptr{Cvoid}, what::Integer, ::Type{T}) where T
    buf = RefValue{T}()
    ret = ccall((:pcre2_pattern_info_8, libpcre2), Cint,
...
function ovec_length(match_data)
    n = ccall((:pcre2_get_ovector_count_8, libpcre2), UInt32,
              (Ptr{Cvoid},), match_data)
    return 2Int(n)
end

function ovec_ptr(match_data)
    ptr = ccall((:pcre2_get_ovector_pointer_8, libpcre2), Ptr{Csize_t},
                (Ptr{Cvoid},), match_data)
    return ptr
end
...
function compile(pattern::AbstractString, options::Integer)
    ...
    re_ptr = ccall((:pcre2_compile_8, libpcre2), Ptr{Cvoid},
...
function jit_compile(regex::Ptr{Cvoid})
    errno = ccall((:pcre2_jit_compile_8, libpcre2), Cint,
...
free_match_data(match_data) =
    ccall((:pcre2_match_data_free_8, libpcre2), Cvoid, (Ptr{Cvoid},), match_data)

free_re(re) =
    ccall((:pcre2_code_free_8, libpcre2), Cvoid, (Ptr{Cvoid},), re)

free_jit_stack(stack) =
    ccall((:pcre2_jit_stack_free_8, libpcre2), Cvoid, (Ptr{Cvoid},), stack)

free_match_context(context) =
    ccall((:pcre2_match_context_free_8, libpcre2), Cvoid, (Ptr{Cvoid},), context)

function err_message(errno::Integer)
    buffer = Vector{UInt8}(undef, 1024)
    ret = ccall((:pcre2_get_error_message_8, libpcre2), Cint,
...
function exec(re, subject, offset, options, match_data)
    ...
    rc = ccall((:pcre2_match_8, libpcre2), Cint,
...
function create_match_data(re)
    p = ccall((:pcre2_match_data_create_from_pattern_8, libpcre2),
...
function substring_number_from_name(re, name)
    n = ccall((:pcre2_substring_number_from_name_8, libpcre2), Cint,
...
function substring_length_bynumber(match_data, number)
    s = RefValue{Csize_t}()
    rc = ccall((:pcre2_substring_length_bynumber_8, libpcre2), Cint,
...
function substring_copy_bynumber(match_data, number, buf, buf_size)
    ...
    rc = ccall((:pcre2_substring_copy_bynumber_8, libpcre2), Cint,
